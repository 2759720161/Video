                                         ...
                                         void *userdata)
{
    ...
    if (OH_AVFormat_GetIntValue(format, OH_MD_KEY_PIXEL_FORMAT, &n)) {
        ...
        /*
         * OHCodec exposes 10-bit semi-planar output through the NV12 enum in
         * buffer mode on current HarmonyOS devices.  The actual storage is
         * P010 (16 bits per component, the 10 useful bits in the MSBs).  If it
         * is advertised to libavutil as 8-bit NV12, every row and chroma plane
         * is interpreted at half its real byte width, producing the familiar
         * green/purple scrambled picture.
         *
         * The coded profile is known before the first output callback, so use
         * it to describe Main-10 HEVC buffers correctly.  Surface mode remains
         * opaque and is intentionally unchanged.
         */
        if (!s->output_to_window && s->pix_fmt == AV_PIXEL_FORMAT_NV12 &&
    ...
    av_log(avctx, AV_LOG_INFO,
           "OHCodec output: coded=%dx%d stride=%d slice_height=%d oh_pix_fmt=%d ff_pix_fmt=%s profile=%d bits=%d\n",
           s->width, s->height, s->stride, s->slice_height, s->pix_fmt,
           av_get_pix_fmt_name(avctx->pix_fmt), avctx->profile,
                                    ...
                                    const OH_AVCodecBufferAttr *attr)
{
    ...
    int32_t capacity = OH_AVBuffer_GetCapacity(output->buffer);
    if (!p || capacity <= 0) {
        av_log(avctx, AV_LOG_ERROR, "Failed to get output buffer addr\n");
        return AVERROR_EXTERNAL;
    }

    /* OH_MD_KEY_VIDEO_STRIDE is a byte stride, not a pixel width.  Some vendor
     * decoders report Main-10 output as AV_PIXEL_FORMAT_NV12 and leave
     * profile/bits_per_raw_sample unset.  A byte stride of at least two bytes
     * per coded pixel identifies the returned semi-planar buffer as P010. */
    if (attr->offset < 0 || attr->size < 0 || attr->offset > capacity ||
        (int64_t)attr->offset + attr->size > capacity) {
        av_log(avctx, AV_LOG_ERROR,
               "Invalid OHCodec buffer attr: capacity=%d offset=%d size=%d\n",
               capacity, attr->offset, attr->size);
        return AVERROR_INVALIDDATA;
    }
    p += attr->offset;
    capacity -= attr->offset;

    int64_t source_bytes = (int64_t)s->stride * s->slice_height * 3 / 2;
    int64_t reported_bytes = FFMAX((int64_t)capacity, (int64_t)attr->size);
    if (s->pix_fmt == AV_PIXEL_FORMAT_NV12 &&
        avctx->pix_fmt == AV_PIX_FMT_NV12 &&
        s->stride >= s->width * 2 &&
        reported_bytes >= source_bytes)
        avctx->pix_fmt = AV_PIX_FMT_P010;

    if (!s->logged_output_layout) {
        av_log(avctx, AV_LOG_INFO,
               "OHCodec output layout: capacity=%d attr_offset=%d attr_size=%d stride_bytes=%d slice_height=%d source_bytes=%" PRId64 " oh_pix_fmt=%d selected=%s profile=%d bits=%d\n",
               capacity, attr->offset, attr->size, s->stride, s->slice_height,
               source_bytes, s->pix_fmt, av_get_pix_fmt_name(avctx->pix_fmt),
               avctx->profile, avctx->bits_per_raw_sample);
        s->logged_output_layout = true;
    }
    ...
    frame->height = s->height;
    /*
     * Keep buffer-mode output equivalent to the hardware-frame path above.
     * In particular, this copies the decoder color description and coded side
     * data (mastering display metadata, content light level and HDR10+ data)
     * onto the AVFrame.  Without it, P010 pixels reach mpv but are treated as
     * untagged video, so PQ/HLG material is rendered with the wrong transfer
     * curve and appears washed out or over-exposed.
     */
    int ret = ff_decode_frame_props(avctx, frame);
    if (ret < 0)
        return ret;
    ...
    if (frame->format == AV_PIX_FMT_NV12 ||
        ...
        src_linesizes[0] = s->stride;
        src_linesizes[1] = s->stride;
        src[0] = p;
        src[1] = p + (int64_t)s->stride * s->slice_height;
        required = source_bytes;
    } else if (frame->format == AV_PIX_FMT_YUV420P ||
               frame->format == AV_PIX_FMT_YUVJ420P) {
        int chroma_stride = (s->stride + 1) / 2;
        int chroma_height = (s->slice_height + 1) / 2;

        src_linesizes[0] = s->stride;
        src_linesizes[1] = chroma_stride;
        src_linesizes[2] = chroma_stride;
        src[0] = p;
        src[1] = p + (int64_t)s->stride * s->slice_height;
        src[2] = src[1] + (int64_t)chroma_stride * chroma_height;
        required = (int64_t)s->stride * s->slice_height +
                   (int64_t)chroma_stride * chroma_height * 2;
    } else {
        av_log(avctx, AV_LOG_ERROR,
               "Unsupported OHCodec software output format: %s\n",
    ...
    if (required < 0 || capacity < required) {
        av_log(avctx, AV_LOG_ERROR,
               "OHCodec output buffer too small: format=%s capacity=%d required=%" PRId64 " stride=%d slice_height=%d\n",
               av_get_pix_fmt_name(frame->format), capacity, required,
               s->stride, s->slice_height);
        return AVERROR_INVALIDDATA;
    }
