...
static int reconfig(struct vo *vo, struct mp_image_params *params)
{
    ...
    /* Allow platform swapchains to select an output transfer function and
     * color gamut from the source metadata before the GPU renderer chooses its
     * target. This is required for dynamic SDR/HDR switching on HarmonyOS. */
    if (sw->fns->set_color) {
        struct mp_image_params target = *params;
        sw->fns->set_color(sw, &target);
    }
