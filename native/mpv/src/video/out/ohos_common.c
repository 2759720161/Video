...
{
    // dex2oat/FFmpeg does not export the private discard helper referenced by
    // an earlier version of this fork.  Keeping that unresolved symbol makes
    // HarmonyOS reject libmpv.so at load time.  The active GPU VO uses
    // ohcodec-copy frames and never enters this surface-only path.
    (void)mpi;
}
